...
def generate_message_h(directory, m):
    ...
    t.write(f, '''
#ifndef MAVLINK_MSG_${name}_${id}
#define MAVLINK_MSG_${name}_${id}
#include <stdint.h>
...
extern inline uint16_t mavlink_msg_${name_lower}_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                              ${{arg_fields: ${array_const}${type} ${array_prefix}${name},}});

extern inline uint16_t mavlink_msg_${name_lower}_pack_chan(uint8_t system_id, uint8_t component_id, uint8_t chan,
                               mavlink_message_t* msg,
                                   ${{arg_fields:${array_const}${type} ${array_prefix}${name},}});

extern inline uint16_t mavlink_msg_${name_lower}_encode(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, const mavlink_${name_lower}_t* ${name_lower});

extern inline uint16_t mavlink_msg_${name_lower}_encode_chan(uint8_t system_id, uint8_t component_id, uint8_t chan, mavlink_message_t* msg, const mavlink_${name_lower}_t* ${name_lower});

${{fields:
extern inline ${return_type} mavlink_msg_${name_lower}_get_${name}(const mavlink_message_t* msg${get_arg});
}}

extern inline void mavlink_msg_${name_lower}_decode(const mavlink_message_t* msg, mavlink_${name_lower}_t* ${name_lower});
#endif
''', m)
    f.close()

def generate_message_c(directory, m):
    '''generate per-message header for a XML file'''
    f = open(os.path.join(directory, 'mavlink_msg_%s.c' % m.name_lower), mode='w')
    t.write(f, '''
#include "../Headers/mavlink_msg_${name_lower}.h"
#include "../Headers/mavlink.h"
#include "../protocol.h"
#include "../mavlink_helpers.h"
inline uint16_t mavlink_msg_${name_lower}_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                              ${{arg_fields: ${array_const}${type} ${array_prefix}${name},}})
{
 ...
 */
inline uint16_t mavlink_msg_${name_lower}_pack_chan(uint8_t system_id, uint8_t component_id, uint8_t chan,
                               mavlink_message_t* msg,
                                   ${{arg_fields:${array_const}${type} ${array_prefix}${name},}})
{
${{scalar_fields:    _mav_put_${type}(msg->payloads, ${wire_offset}, ${putname});
 ...
 */
inline uint16_t mavlink_msg_${name_lower}_encode(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, const mavlink_${name_lower}_t* ${name_lower})
{
 ...
 * @param ${name_lower} C-struct to read the message contents from
 */
inline uint16_t mavlink_msg_${name_lower}_encode_chan(uint8_t system_id, uint8_t component_id, uint8_t chan, mavlink_message_t* msg, const mavlink_${name_lower}_t* ${name_lower})
{
 ...
 */
inline ${return_type} mavlink_msg_${name_lower}_get_${name}(const mavlink_message_t* msg${get_arg})
{
 ...
 */
inline void mavlink_msg_${name_lower}_decode(const mavlink_message_t* msg, mavlink_${name_lower}_t* ${name_lower})
{
...
def generate_one(basename, xml):
    ...
    headers_dir = os.path.join(basename,'Headers')
    sources_dir = os.path.join(basename,'Sources')

    print("Generating C implementation in directory %s" % directory)
    mavparse.mkdir_p(directory)
    mavparse.mkdir_p(headers_dir)
    mavparse.mkdir_p(sources_dir)
    ...
    generate_mavlink_h(headers_dir, xml)
    generate_version_h(headers_dir, xml)
    generate_main_h(headers_dir, xml)
    for m in xml.message:
        generate_message_h(headers_dir, m)
        generate_message_c(sources_dir,m)
    generate_testsuite_h(headers_dir, xml)
