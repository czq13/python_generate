...
from msg_pyutils.msgparse import MSGParseError
...
class MAVTemplate(object):
    ...
    def find_end(self, text, start_token, end_token, ignore_end_token=None):
        ...
        if not text.startswith(start_token):
            raise MSGParseError("invalid token start")
        ...
        while nesting > 0:
            ...
            if idx1 == -1 and idx2 == -1:
                raise MSGParseError("token nesting error")
    ...
    def substitute(self, text, subvars={},
        ...
        while True:
            ...
            if endidx == -1:
                raise MSGParseError("missing end macro in %s" % text[subidx:])
            part1 = text[0:subidx]
            ...
            if v is None:
                raise MSGParseError('unable to find field %s' % field_name)
            t1 = part1
            for f in v:
        ...
        while True:
            ...
            if endidx == -1:
                raise MSGParseError('missing end of variable: %s' % text[idx:idx+10])
            varname = text[idx+2:idx+endidx]
            if isinstance(subvars, dict):
                if not varname in subvars:
                    if checkmissing:
                        raise MSGParseError("unknown variable in '%s%s%s'" % (
